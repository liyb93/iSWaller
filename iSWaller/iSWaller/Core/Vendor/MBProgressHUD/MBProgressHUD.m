...
#else   // (TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE)
    label = [[NSTextField alloc] initWithFrame:self.bounds];
//	 label.adjustsFontSizeToFitWidth = NO;
    label.editable = NO;
    ...
    label.bordered = NO;
    label.bezeled = NO;
	if (self.labelText) label.stringValue = self.labelText;
#endif  // (TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE)
...
#else   // (TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE)
	detailsLabel = [[NSTextField alloc] initWithFrame:self.bounds];
	detailsLabel.font = self.detailsLabelFont;
	// detailsLabel.adjustsFontSizeToFitWidth = NO;
	...
	// detailsLabel.numberOfLines = 0;
	detailsLabel.font = self.detailsLabelFont;
	if (self.detailsLabelText) detailsLabel.stringValue = self.detailsLabelText;
#endif  // (TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE)
...
#else   // !(TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE)
    CGSize labelSize = MB_TEXTSIZE(label.stringValue, label.font);
    if (labelSize.width > 0.0f) {
        labelSize.width += 20.0f;
...
#else   // !(TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE)
    CGSize detailsLabelSize = MB_TEXTSIZE(detailsLabel.stringValue, detailsLabel.font);
...
- (void)updateUIForKeypath:(NSString *)keyPath
{
	if ([keyPath isEqualToString:@"mode"] || [keyPath isEqualToString:@"customView"]) {
		...
		detailsLabel.text = self.detailsLabelText;
#else   // !(TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE)
        detailsLabel.stringValue = self.detailsLabelText;
#endif  // (TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE)
